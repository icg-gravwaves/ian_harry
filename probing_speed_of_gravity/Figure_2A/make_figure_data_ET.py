...
# This code takes a while to run as ET waveforms are long! Could have been
# better with a parallel for loop (as we did for Figure 3).
...
non_gr_n_values = np.zeros([19])
non_gr_n_values[:9] = np.linspace(1, 2, 9)
...
# SNR scaling factor computed by doing:
# sigma = pycbc.filter.sigma(hp1, psd=psdl, low_frequency_cutoff=XX)
# for both ET and LIGO and then putting both numbers in as a ratio
snr = 32.4 * (1040.5166847798146 / 85.95844388879294)
...
for ndx, nval in enumerate(non_gr_n_values):
    for sign in [-1, 1]:
        ...
        overlap1, _ = pycbc.filter.match(hp1, hp2, psd=psdl, low_frequency_cutoff=1)
        # print(overlap1)
        if overlap1 < dist_overlap:
            while overlap1 < dist_overlap:
                ...
                overlap1, _ = pycbc.filter.match(
                    hp1, hp2, psd=psdl, low_frequency_cutoff=1
                )
                if curr_a_at_30 < 1e-25:
                    break
            if curr_a_at_30 < 1e-25:
                outputs[ndx] = curr_a_at_30
                continue
